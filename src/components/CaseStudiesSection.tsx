...
export const CaseStudiesSection: React.FC = () => {
  ...
  const studies = getEvidenceCaseStudies();
  ...
  const filteredStudies = studies.filter(study => {
    if (activeCategory !== 'all' && study.category !== activeCategory) return false;
    if (searchQuery.trim() !== '') {
  ...
  const getStatusBadge = (study: CaseStudy) => {
    const status = study.deploymentStatus;
    ...
    if (status === 'validated') {
      return (
        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-800 border border-blue-300 text-[11px] font-mono font-bold">
          ...
          VALIDATED / NOT DEPLOYMENT CLAIM
        </span>
      );
    }
  ...
  return (
    <section className="py-12 lg:py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10 font-sans">
      <div className="space-y-3 max-w-3xl">
        ...
        <p className="text-sm sm:text-base text-[#4A4A45] leading-relaxed">{t('projects.subtitle')}</p>
        <p className="text-xs text-[#70706B] font-mono">Lifecycle status is separate from implementation status: production is shown only when explicit deployment evidence exists.</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        <div className="flex flex-wrap gap-1.5 p-1 bg-[#F4F4F1] rounded-lg border border-[#D1D1CD]">
          {categoryFilters.map(cat => (
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-mono transition-all cursor-pointer ${activeCategory === cat.id ? 'bg-[#1A1A1A] text-white font-semibold shadow-xs' : 'text-[#70706B] hover:text-[#1A1A1A]'}`}
            >
              {cat.label}
            </button>
          ))}
        ...
        <div className="relative min-w-[240px]">
          ...
          <input
            ...
            placeholder="Search by tech (Java, Python, TypeScript, Spring Boot)..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredStudies.map((study) => (
          <div key={study.id} className="rounded-xl border border-[#D1D1CD] bg-white hover:border-[#1A1A1A] transition-all p-6 flex flex-col justify-between space-y-5 group shadow-xs">
            <div className="space-y-3.5">
              <div className="flex items-center justify-between text-xs gap-2">
                <span className="text-[#1A1A1A] font-mono text-[11px] font-semibold px-2 py-0.5 rounded bg-[#F4F4F1] border border-[#D1D1CD]">{study.tag}</span>
                {getStatusBadge(study)}
              </div>

              <div>
                <h3 className="text-base sm:text-lg font-bold text-[#1A1A1A] font-sans group-hover:text-black">{study.title}</h3>
                <p className="text-xs text-[#70706B] font-mono pt-0.5">{study.sector}</p>
              </div>

              <p className="text-xs text-[#4A4A45] leading-relaxed font-sans line-clamp-3">{study.subtitle}</p>
              ...
              <div className="p-3 rounded-lg bg-[#F4F4F1] border border-[#D1D1CD] space-y-1">
                ...
                <p className="text-xs text-[#4A4A45] font-sans leading-relaxed">{study.whatItProves}</p>
              </div>
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <div className="flex flex-wrap gap-1">
                  {study.realTechnologies.languages.concat(study.realTechnologies.frameworks.slice(0, 2)).map((tItem, idx) => (
                    <span key={idx} className="px-1.5 py-0.5 rounded bg-[#F4F4F1] border border-[#D1D1CD] text-[10px] font-mono text-[#1A1A1A]">{tItem}</span>
                  ))}
                  {study.realTechnologies.databases.length > 0 && (
                    <span className="px-1.5 py-0.5 rounded bg-[#F4F4F1] border border-[#D1D1CD] text-[10px] font-mono text-[#70706B]">{study.realTechnologies.databases[0]}</span>
                  )}
                </div>
              </div>
            ...
            <button onClick={() => setSelectedStudy(study)} className="w-full mt-2 py-2 px-3 rounded-lg bg-[#F4F4F1] hover:bg-[#1A1A1A] text-[#1A1A1A] hover:text-white border border-[#D1D1CD] hover:border-[#1A1A1A] text-xs font-mono font-semibold transition-all flex items-center justify-center gap-2 cursor-pointer group-hover:bg-[#1A1A1A] group-hover:text-white">
      ...
      {filteredStudies.length === 0 && (
        <div className="p-12 text-center rounded-xl border border-dashed border-[#D1D1CD] bg-white space-y-2">
          ...
          <button onClick={() => { setActiveCategory('all'); setSearchQuery(''); }} className="text-xs font-mono text-[#1A1A1A] underline cursor-pointer">Reset filters</button>
        </div>
      )}

      <RepositoryTruthModal study={selectedStudy} onClose={() => setSelectedStudy(null)} />
    </section>
  );
};
